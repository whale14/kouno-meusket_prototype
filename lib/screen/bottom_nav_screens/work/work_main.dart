...
class _BodyHelperState extends State<BodyHelper> {
  ...
  Future _initialize() async {
    await _getMyIdx();
    await _requestViewModel.onRequestEvent(RequestEvent.getRequests(myIdx!));
                              ...
                              builder: (context) {
                                return AlertDialog(
                                  ...
                                  actions: [
                                    TextButton(
                                        onPressed: () async{
                                          await _requestViewModel.onRequestEvent(RequestEvent.acceptRequest(request.idx.toString(), myIdx!));
    ...
    if(user.doingState == 0) {
      return Container(
        ...
        child: Container(
          ...
          child: Text("활동중", style: TextStyle(color: Colors.white),),
        ),
      );
    } else {
      return Container(
