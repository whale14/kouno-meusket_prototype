...
class WalletViewModel with ChangeNotifier {
  ...
  Future onWalletEvent(WalletEvent event) async {
    ...
    switch(event){
      ...
      case GetIncomeHistory(): _getIncomeHistory(event.idx, event.month, event.type);
      case GetDepositHistory(): _getDepositHistory(event.idx, event.month, event.type);
    }
  }
  ...
  Future _getDepositHistory(String idx, String month, String type) async{
    final result = _walletRepository.getDepositHistory(idx, month, type);
