...
class WalletViewModel with ChangeNotifier {
  ...
  Future onWalletEvent(WalletEvent event) async {
    switch (event) {
      ...
      case GetIncomeHistory():
        _getIncomeHistory(event.idx, event.month, event.type);
      case GetDepositHistory():
        _getDepositHistory(event.idx, event.month, event.type);
    }
  }
  ...
  Future _getIncomeHistory(String idx, String month, String type) async {
    final result = _walletRepository.getIncomeHistory(idx, month, month);
    _walletState = walletState.copyWith(incomes: await result);
    notifyListeners();
  }

  Future _getDepositHistory(String idx, String month, String type) async {
    _walletState = walletState.copyWith(isLoading: true);
    notifyListeners();
    final result = await _walletRepository.getDepositHistory(idx, month, type);
    _walletState = walletState.copyWith(isLoading: false, deposits: result);
    notifyListeners();
  }
}
